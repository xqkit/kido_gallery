...
public class MagicCameraView extends MagicBaseView {
            ...
            switch (recordingStatus) {
                case RECORDING_OFF:
                    ...
                    videoEncoder.startRecording(new TextureMovieEncoder.EncoderConfig(
                            Utils.getOutputMovie(), info.previewWidth, info.pictureHeight,
                            125000, EGL14.eglGetCurrentContext(),
                            info));
    ...
    public void savePicture(final SavePictureTask savePictureTask) {
        CameraEngine.takePicture(null, null, new Camera.PictureCallback() {
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                Log.d("frank","data = " + data.length);
                final Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
                Log.d("frank"," onPictureTaken bitmap = " + bitmap.toString());
                queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        Log.d("frank","bitmap = " + bitmap.toString());
                        final Bitmap photo = drawPhoto(bitmap, CameraEngine.getCameraInfo().isFront);
                        Log.d("frank","photo = " + photo.getByteCount());
    ...
    private Bitmap drawPhoto(Bitmap bitmap, boolean isRotated) {
        ...
        if (filter != null) {
            Log.d("frank", "filter draw photo " + filter.toString());
        ...
        beautyFilter = null;
        if (filter != null) {
            Log.d("frank", "filter != null" + filter.toString());
