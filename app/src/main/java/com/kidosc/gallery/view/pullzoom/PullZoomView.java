...
//        if (oldt > t && oldt - t > 0) {// 向下
//            Log.d(TAG,"向下");
//        } else if (oldt < t && t - oldt > 0) {// 向上
//            Log.d(TAG,"向上");
//        }
//    }
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (t >= 0 && t <= maxY) {
            scrollFlag = true;
            Log.d(TAG, "t = " + t + " maxY = " + maxY);
            Log.d(TAG, "t/maxY = " + (float) t / maxY);
            headerView.setAlpha((float) (maxY - t) / maxY);
            ...
            if (t < 0) t = 0;
            if (t > maxY) t = maxY;
            Log.d(TAG, " scrollFlag = true;t = " + t + " maxY = " + maxY);
            Log.d(TAG, "scrollFlag = true;t/maxY = " + t / maxY);
            if (scrollListener != null) scrollListener.onHeaderScroll(t, maxY);

        }
        if (t >= maxY) {
            Log.d(TAG, " t> = maxY;t = " + t + " maxY = " + maxY);
            Log.d(TAG, "t> = maxY;t/maxY = " + t / maxY);
